...
bot.on('presenceUpdate', (oldMember, newMember) => { // Set town Offline
  if (newMember.presence.status === 'offline') {
    dbAccess.setOfflineTown(newMember.id, [(offline, data) => {
      bot.channels.get(data.botConfig.channelID.online).send(`<@${data.newMember.id}>'s Town has been set offline automatically! *(The user is offline on Discord) \n @here*`)
    ...
    }, { 'newMember' : newMember, 'botConfig' : botConfig}])
  }
})
