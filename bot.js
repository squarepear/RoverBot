...
const userpattern = new RegExp(/<@\d*>/g);
...
function setUserInfo(userID, info) {
    ...
    if (row == null) {
      ...
      row = db.prepare(`SELECT * FROM USERINFO WHERE UserID=?`).get(userID);
		...
		if (info.FriendCode != null)
			row.FriendCode = info.FriendCode.trim()
		if (info.Name != null)
			row.Name = info.Name.trim()
		if (info.Town != null)
			row.Town = info.Town.trim()
		if (info.Fruit != null)
			row.Fruit = info.Fruit.trim()
		if (info.Note != null)
			row.Note = info.Note.trim()
